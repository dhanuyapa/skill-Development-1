...
if(isset($_POST['virtusa']))
{
    $valueToSearch = 'Virtusa';
    $position = $_POST['position'];	
}
else if(isset($_POST['fortude']))
{
    $valueToSearch = 'Fortude';
    $position = $_POST['position'];	
}
else if(isset($_POST['hcl']))
{
    $valueToSearch = 'HCL';
    $position = $_POST['position'];	
}
else if(isset($_POST['wso2']))
{
    $valueToSearch = 'WSO2';
    $position = $_POST['position'];	
}
...
if($valueToSearch != null && $position != null){
	...
	$query = "SELECT * FROM `online_jobs` where companyName='".$valueToSearch."' AND position = '".$position."'";
    $search_result = filterTable($query, $con);	
}
else if($valueToSearch == null && $position != null){
	$query = "SELECT * FROM `online_jobs` where position = '".$position."'";
    $search_result = filterTable($query, $con);
}
else if($valueToSearch != null && $position == null){
	
	$query = "SELECT * FROM `online_jobs` where companyName='".$valueToSearch."'";
    $search_result = filterTable($query, $con);	
}
else{
	$query = "SELECT * FROM `online_jobs`";
    $search_result = filterTable($query, $con);
}
?>
...
<html>
    <head>
        <link rel="stylesheet" type="text/css" href="styles/online_jobs.css">
		
        <title>
           Online Jobs
        </title>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel = "stylesheet" href = "online_jobs.css">

    </head>
   <body style="background-image:url('images/onlineback.webp');">
       <header class="header">
		...
		<form action="online_Jobs.php" method="post">
        
            
		<div align = "center">
				<h3>Search Jobs for Top Companies</h3>
				<button type="submit" name="virtusa" id = "virtusa"><img src="images/virtusa.jpg" class="l1" alt = "Virtusa"></button>
				<button type="submit" name="hcl", id = "hcl" value="HCL"><img src="images/hcl.png" class="l2"></button>
				<button type="submit" name="fortude", id = "fortude" value="FORTUDE"><img src="images/fortude.png" class="l2"></button>
				<button type="submit" name="wso2", id = "wso2" value="WSO2"><img src="images/wso2.png" class="l2"></button>
      ...
      <!-- populate table from mysql database -->
                ...
                <center><h1>Online Job Companies</h1></center>
	</form>
